...
public class UIManager : MonoBehaviour
{
	...
	private void CreatePool<T>(Transform canvasParent, T prefab, ref T[] array, int count, string parentName) where T : UnityEngine.Component
	{
		GameObject parent = new GameObject(parentName);
		parent.transform.SetParent(canvasParent);
		array = new T[count];
		for (int i = 0; i < count; ++i)
		{
	...
	private void LateUpdate()
	{
		...
		{
			_anims[i].AnimPercentage += deltaAnim; // increment animation
            _anims[i].AnimPercentage = Mathf.Clamp01(_anims[i].AnimPercentage);
			ApplyAnim(_anims[i]);
	...
	private void ApplyAnim(SpacecraftUIAnim anim)
	{
		...
		if (!anim.Arrival)
		{ 
            // for departures, start the anim at the very right of the screen (offscreen)
			startX = Screen.width;
	...
	public void StartFlight(Spacecraft craft, bool arrival, float timerPercentage = 1.0f)
	{
		...
		array[index].Populate(craft, arrival);
	...
	public void RemoveSpacecraft(Spacecraft craft, bool arrival)
	{
        ...
        {
            // Reverse animation
            int animIndex = _anims.FindIndex(x => x.Craft == craft);
            if (animIndex >= 0)
            {
	...
	public void UpdateSpacecraft(Spacecraft craft, float timerPercentage, bool arrival)
	{
		...
		{
			List<QueuedSpacecraftUI> queue = arrival ? _queuedArrivals : _queuedDepartures;
            i = queue.FindIndex(x => x.Craft == craft);
            if (i >= 0)
            {
